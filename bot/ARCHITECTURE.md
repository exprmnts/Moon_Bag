...
**`retry.js`** → every `RETRY_TICK_MS` (10 s), `executor.due()` selects `retrying` rows whose `next_retry_at` has passed and runs each through the same `attempt`. At boot it also calls `reclaimStranded()`, which puts back in the queue any row a crash left `pending`, and `requeueOperatorFixable()`, which puts back anything that failed on **our** misconfiguration — `errors.OPERATOR_FIXABLE` (`CONFIG`, `NOT_MAINNET`, `KEY_MISMATCH`) inside `BOOT_REQUEUE_WINDOW` (1 hour). A restart is the only event that can change those outcomes, so it is the signal that the fix landed; nobody should have to type `/retry` to recover from an outage they did not cause. Narrow on purpose — these fire without anyone confirming the spend, so an older signal stays behind `/retry`, where the user is shown the ETH first. All three codes fail before any ETH moves, so a restart into a still-broken config costs one log line per row and nothing else.
