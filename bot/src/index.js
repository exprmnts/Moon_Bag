...
bot.action("SET_BUY_AMOUNT", async (ctx) => {
  ...
  await ctx.reply(
    `💰 <b>Set Buy Amount</b>\n\nSend me the ETH amount to spend per auto-buy (e.g., 0.005, 0.01, 0.05).\n\nCurrent: <b>${formatEther(user.buyAmountWei)} ETH</b>\n\n<i>This is how much ETH I'll spend each time I detect a sell signal!</i>`,
    html()
  );
});
...
bot.on(message("text"), async (ctx) => {
  ...
  if (awaiting === "buy_amount") {
    ...
    await ctx.reply(
      `✅ <b>Buy Amount Updated!</b>\n\nI'll now spend <b>${value} ETH</b> per auto-buy when I detect sell signals. 🚀`,
      withMenu()
    );
    return;
  }
...
bot.action("REMOVE_WATCH_ADDR", async (ctx) => {
  ...
  if (!list.length) {
    return ctx.reply("❌ <b>No Watch Addresses</b>\n\nYou don't have any addresses to remove.", withMenu());
  }
...
bot.action("CHECK_BALANCE", async (ctx) => {
  ...
  try {
    ...
    if (tokens.length) {
      ...
      text += "🪙 <b>Token Balances:</b> No tokens found";
    }
...
bot.action("SHOW_POSITIONS", async (ctx) => {
  ...
  if (!list.length) {
    return ctx.reply("👀 <b>No watch addresses configured</b>\n\nAdd external wallet addresses to monitor their positions.", withMenu());
  }
  let text = "📊 <b>Monitored Positions</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n";
  for (let i = 0; i < list.length; i++) {
    const w = list[i];
    text += `📍 <b>Address ${i + 1}:</b>\n<code>${w.address}</code>\n`;
    try {
