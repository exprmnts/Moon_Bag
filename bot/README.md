...
New service from GitHub `exprmnts/Moon_Bag`, root directory `bot`, Dockerfile build, one replica, health check path `/health`. Variables: everything in `.env.example` with the Neon `main` branch string, `CHAIN=mainnet`, `DRY_RUN=false` (or `true` to observe first). Only one instance may run at a time: Telegram long-polling and the WSS subscription both assume a single process.
