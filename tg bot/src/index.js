...
bot.on("text", async (ctx) => {
  ...
  if (
    ...
    await ctx.reply(`💸 Buy amount set to <b>${value}</b> SOL.`, {
      ...mainKeyboard(),
      parse_mode: "HTML",
    });
  } else if (
...
bot.action("START_WATCH", async (ctx) => {
  try {
    ...
    await ctx.reply(
      "▶️ Watching started — I’ll notify you of sells and auto-buys!",
      mainKeyboard()
    );
  } catch (err) {
    console.error(err);
    ...
    await ctx.reply(
      "Could not start watcher. Ensure you have a funded wallet."
    );
  }
});
